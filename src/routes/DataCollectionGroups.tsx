...
import { useNavigate, useLoaderData, useParams, Link as LinkRouter } from "react-router-dom";
...
const DataCollectionGroups = () => {
  const dataCollectionGroups = useLoaderData() as { [key: string]: DataCollectionGroup };
  const { sessid } = useParams();
  const navigate = useNavigate();

  const SelectDataCollectionGroup = async (data: Record<string, any>, index: number) => {
    navigate(`/sessions/${sessid}/data_collection_groups/${data["id"]}/grid_squares`);
  };
