...
const ProcessingParameters = () => {
  ...
  (procParams?.map((p) => {
    ...
    Object.entries(p?.relion_params ? p?.relion_params: {}).forEach(([key, value]) => tr.processingRows.push({"parameterName": nameLabelMap.get(key)??key, "parameterValue": value === true ? "True": value === false ? "False": value}));
    tr.tag = p?.relion_params.pj_id.toString();
    tableRows.push(tr);
    Object.entries(p?.feedback_params ? p?.feedback_params: {}).forEach(([key, value]) => tre.processingRows.push({"parameterName": nameLabelMap.get(key)??key, "parameterValue": value === true ? "True": value === false ? "False": value}));
    ...
    <Box mt='-1em' mx='-7.3vw' bg='murfey.50' flex='1 0 auto'>
        ...
        {tableRows.map((tr) => {return (
            <AccordionItem>
                ...
                <AccordionPanel>
                <Table data={tr.processingRows} headers={[{'key': 'parameterName', 'label': 'Parameter'}, {'key': 'parameterValue', 'label': 'Value'}]} label={'processingParameterData'} /> 
                </AccordionPanel>
            </AccordionItem>
        )})}
        {showExtra ? tableRowsExtra.map((tre) => {return (
             <AccordionItem>
             ...
             <AccordionPanel>
                <IconButton aria-label="Edit parameters" icon={<MdEditNote/>} />
                <Table data={tre.processingRows} headers={[{'key': 'parameterName', 'label': 'Parameter'}, {'key': 'parameterValue', 'label': 'Value'}]} label={'processingParameterData'} /> 
            </AccordionPanel>
            </AccordionItem>
        )}): <></>}
        </Accordion>
