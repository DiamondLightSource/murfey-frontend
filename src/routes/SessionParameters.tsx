import {
  ...
  const SessionParameters = () => {
    ...
    type EditableParameter = "gain_ref" | "dose_per_frame" | "eer_fractionation_file" | "symmetry" | "";
    ...
    Object.entries(sessionParams ? sessionParams: {}).forEach(
      ([key, value]) =>
        tableRows.push({
          parameterName: nameLabelMap.get(key) ?? key,
          parameterValue: value,
          parameterKey: key} as ProcessingRow,
    ))
    ...
    const editParameterDialogue = async (data: Record<string, any>, index: number) => {
    ...
    return (
      <div className="rootContainer">
        ...
        <ModalContent>
          ...
          <ModalBody>
            {paramName}
            <Input value={paramValue} autoFocus w="80%" onChange={(v) => setParamValue(v.target.value)}/>
          </ModalBody>
          <ModalFooter>
            <Button colorScheme="blue" mr={3} onClick={onClose}>
              Close
            </Button>
            <Button variant="ghost" onClick={() => handleParameterEdit()}>Confirm</Button>
          </ModalFooter>
        </ModalContent>
        </Modal>
