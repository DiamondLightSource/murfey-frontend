...
export const linkSessionToClient = async (client_id: number, sessionName: string) => {
  const response = await client.post(`clients/${client_id}/session`, { "session_name": sessionName });
  if (response.status !== 200) {
    return null;
  }
  return response.data;
...
export const deleteSessionData = async (sessid: number) => {
  ...
  export const clientsLoader = (queryClient: QueryClient) => async () =>
    (await queryClient.getQueryData(clientQuery.queryKey)) ?? (await queryClient.fetchQuery(clientQuery));
